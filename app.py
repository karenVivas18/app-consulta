...
def procesar_dump_accounts(texto):
    ...
    if not registros:
        return "-- ⚠️ No se detectaron INSERTS válidos de M_DUMP_DEBIT_CARD."
    
    queries = []
    for reg in registros:
        valores = [v.strip().strip("'") for v in reg.split(",")]
        try:
            # Según tu estructura: ID es pos 0, ACCOUNT_NUMBER es pos 16
            dump_id = valores[0]
            acc_number = valores[16]
            q = f"INSERT INTO M_DUMP_DEBIT_ACCOUNTS (MDUMP_ID, ACCOUNT_TYPE, ACCOUNT_NUMBER, ACCOUNT_STATUS, ACCOUNT_PREFERRED, ACCOUNT_PRIMARY) VALUES ({dump_id}, '1', '{acc_number}', '1', '0', '1');"
            queries.append(q)
        except IndexError:
            continue
    return "\n".join(queries)
...
tab1, tab2, tab3, tab4 = st.tabs(["📝 Trámites Diarios", "🔧 Varios & Settlement", "⚠️ Eliminación", "📦 Cargas Masivas"])
...
with tab4:
    st.subheader("📦 Generador M_DUMP_DEBIT_ACCOUNTS")
    st.info("Pega los INSERTS de tarjetas y obtén los de cuentas vinculados por ID.")
    input_dump = st.text_area("INSERTS de M_DUMP_DEBIT_CARD:", height=250, placeholder="INSERT INTO M_DUMP_DEBIT_CARD VALUES (1, ...")
    if st.button("Generar Inserts de Cuentas"):
        if input_dump:
            st.code(procesar_dump_accounts(input_dump), language="sql")